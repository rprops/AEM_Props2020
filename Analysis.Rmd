...
# For adding manuscript MAG information etcµ
...
p_desman_overview <- ggplot(metadata_desman, aes(x = Genome_ID, y = number_of_robust_haplotypes, fill = Lineage, group = Genome_ID))+
  theme_bw()+
  geom_bar(alpha = 1, stat = "identity", color = "black",
           position = position_dodge(width = 1), width = 0.7)+
  scale_fill_manual("",
...
for(i in 1:10){
  tmp_desm <- read.csv(paste0("./DESMAN/DESMAN_other_MAGs/", metadata_desman$MAG_id[i], "/",
    metadata_desman$best_desman_run[i],"/Gamma_meanR.csv"))
  # wide to long
...
```{r desman-3, dpi = 500, warning = FALSE, fig.width = 7, fig.height = 10, dev = c("png","pdf")}
# Import data
...
```{r desman-2c, dpi = 500, warning = FALSE, fig.width = 7, fig.height = 7, dev = c("png","pdf")}
