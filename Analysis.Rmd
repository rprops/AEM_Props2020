...
for(i in 1:10){
  results_desm_all_sb <- results_desm_all %>% 
     dplyr::filter(MAG %in% paste0("MAG",i)) %>% 
    dplyr::select(Frequency,`Temperature..C.`, Strain, `Cond..µS.cm.`, DOC.mg.L, PAR, TDP.ug.L, Season) %>% 
  ...
  p.mat <- p.mat %>% dplyr::select(-`Temperature..C.`, -`Cond..µS.cm.`, -`DOC.mg.L`, -PAR, -`TDP.ug.L`) 
  p.mat <- p.mat %>% 
...
p_cor_desman <- p.res %>% 
  dplyr::filter(cor_variable %in% c("Temperature..C.", "Cond..µS.cm.", "DOC.mg.L", "PAR", "TDP.ug.L")) %>% 
  ...
  scale_size("Correlation", range = c(3.5,8), breaks=seq(1,10,by=3)) +
...
png(paste0("./Figures/DESMAN/desman-strain-correlations.png"), height = 6, 
