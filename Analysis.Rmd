...
# run sparcc
res_sparcc <- sparcc(strain_cor, iter = 100, inner_iter = 100, th = 0.1)
...
# Visualize
vsize    <- rowMeans(clr(strain_cor, 1))+6
am.coord <- layout.fruchterman.reingold(ig.sparcc)

plot.igraph(ig.sparcc, layout=am.coord, vertex.size=vsize, vertex.label=NA, main="Strain network",
  vertex.color = "red")
...
for(i in 1:10){
  results_desm_all_sb <- results_desm_all %>% 
     dplyr::filter(MAG %in% paste0("MAG",i)) %>% 
    dplyr::select(Frequency,`Temperature..C.`, Strain, `SpCond..µS.cm.`, DOC.mg.L, PAR, TDP.ug.L, Season) %>% 
  ...
  p.mat <- p.mat %>% dplyr::select(-`Temperature..C.`, -`SpCond..µS.cm.`, -`DOC.mg.L`, -PAR, -`TDP.ug.L`) 
  p.mat <- p.mat %>% 
...
p_cor_desman <- p.res %>% 
  dplyr::filter(cor_variable %in% c("Temperature..C.", "SpCond..µS.cm.", "DOC.mg.L", "PAR", "TDP.ug.L")) %>% 
  dplyr::mutate(cor_variable = factor(cor_variable, levels = c("PAR", "SpCond..µS.cm.", "TDP.ug.L", "DOC.mg.L", "Temperature..C."))) %>% 
...
png(paste0("./Figures/DESMAN/desman-strain-correlations-2.png"), height = 8, 
  width = 12, units = "in", res = 500)
