...
# import metadata
metadata_desman <- readxl::read_excel("./DESMAN/DESMAN_other_MAGs/desman_results.xlsx", sheet = 1)
...
for(i in 1:10){
  tmp_desm <- read.csv(paste0("./DESMAN/DESMAN_other_MAGs/", metadata_desman$MAG_id[i], "/",
  ...
  if(i == 1) results_desm_all <- temp_desm else results_desm_all <- rbind(results_desm_all, temp_desm)
}
...
<!--   dplyr::select(Samples:MAG) %>%  -->
<!--   mutate(Strain_MAG = interaction(Strain, MAG)) %>%  -->
<!--   dplyr::select(-Strain, - MAG) %>%  -->
<!--   spread(., Strain_MAG, Frequency) -->

<!-- alpha_div_desman_all <- data.frame(Sample = df_div_desman_all[,1],  -->
<!--       D2 = vegan::diversity(df_div_desman_all[,2:ncol(df_div_desman_all)], index = "invsimpson") -->
<!-- ) -->

<!-- alpha_div_desman_all <- left_join(alpha_div_desman_all, meta_em, -->
<!--                               by = c("Sample" = "Sample_ID")) -->
<!-- alpha_div_desman_all$Site <- as.character(alpha_div_desman_all$Site) -->
<!-- alpha_div_desman_all$Site <- gsub("110", "Lake Michigan - M110", alpha_div_desman_all$Site) -->
<!-- alpha_div_desman_all$Site <- gsub("15", "Lake Michigan - M15", alpha_div_desman_all$Site) -->
<!-- alpha_div_desman_all$Site <- gsub("Buoy", "Muskegon Lake", alpha_div_desman_all$Site) -->
<!-- alpha_div_desman_all$Site <- factor(alpha_div_desman_all$Site, levels = c("Muskegon Lake", -->
<!--                                                             "Lake Michigan - M15", -->
<!--                                                             "Lake Michigan - M110")) -->
<!-- alpha_div_desman_all$Season <- as.character(alpha_div_desman_all$Season) -->
<!-- alpha_div_desman_all$Season <- factor(alpha_div_desman_all$Season, levels = c("Spring", "Summer","Fall")) -->

<!-- p_alpha_desman_all <- alpha_div_desman_all %>%  -->
<!--   ggplot(., aes(x = Season, y = D2, fill = Season, shape = Depth))+ -->
<!--   geom_point(size = 4) + -->
<!--   scale_shape_manual("", values = c(21,22,24))+ -->
<!--   facet_grid(.~Site)+ -->
<!--   theme_bw()+ -->
<!--   scale_fill_brewer(palette = "Accent")+ -->
<!--   labs(y = expression("Limnohabitans strain diversity (D"[2]*")"), x = "")+ -->
<!--   theme(axis.text=element_text(size=12), axis.title=element_text(size=16), -->
<!--         title=element_text(size=16), legend.text=element_text(size=16), -->
<!--         strip.text = element_text(size = 16))+ -->
<!--   ylim(0,20)+ -->
...
<!-- png(paste0("./Figures/DESMAN/desman-alpha-div-allMAGs.png"), height = 4, -->
<!--   width = 10, res = 500, units = "in") -->
<!-- print(p_alpha_desman_all) -->
<!-- dev.off() -->

<!-- png(paste0("./Figures/DESMAN/desman-alpha-div-allMAGs-combined.png"), height = 8, -->
<!--   width = 10, res = 500, units = "in") -->
<!-- cowplot::plot_grid(p_alpha_desman_all, p_alpha_desman, align = "hv", ncol = 1, labels = "AUTO") -->
<!-- dev.off() -->
<!-- ``` -->
...
Some highly speculative additional analysis over here.
