...
```{r metaT-Zscore, dpi = 500, warning = FALSE, fig.width = 7, fig.height = 5, dev=c("png","pdf")}
...
```{r desman-2a, dpi = 500, warning = FALSE, fig.width = 7, fig.height = 6, dev = c("png","pdf")}
...
```{r desman-2, dpi = 500, warning = FALSE, fig.width = 7, fig.height = 7, dev = c("png","pdf")}
...
```{r desman-site-downreg, dpi = 500, warning = FALSE, fig.width = 8, fig.height = 6, dev = c("png","pdf")}
...
```{r desman-site-upreg, dpi = 500, warning = FALSE, fig.width = 8, fig.height = 6, dev = c("png","pdf")}
...
```{r desman-season-downreg, dpi = 500, warning = FALSE, fig.width = 5, fig.height = 6, dev = c("png","pdf")}
...
```{r desman-depth-downreg, dpi = 500, warning = FALSE, fig.width = 8, fig.height = 6, dev = c("png","pdf")}
...
```{r desman-depth-upreg, dpi = 500, warning = FALSE, fig.width = 8, fig.height = 6, dev = c("png","pdf")}
...
```{r MAG8-DESeq-plot-strains, dpi = 500, warning = FALSE, fig.width = 9, fig.height = 7, dev = c("png","pdf")}
...
```{r irep-6, dpi = 500, warning = FALSE, fig.width = 6, fig.height = 4}
...
hm_DOC2_full <- DOC_panG_cnt_full %>% 
...
```{r PhenoD-1, dpi = 500, warning = FALSE, fig.width = 2, fig.height = 12, include = FALSE}
...
min_reads <- expr_cov_long %>% group_by(Sample) %>% 
                summarize(sample.sums = sum(mapped_reads))
...
for(i in 1:length(unique(expr_cov_pd$Sample))){
  ...
  phylo_expr_cov_pd <- phyloseq::phyloseq(phyloseq::otu_table(phy_tmp[ ,-c(1,2)],
                                                              taxa_are_rows = TRUE),
                                         as.matrix(phyloseq::tax_table(phy_tmp[,1])))
  
  # Rarefy to even depth across samples (5% of 1 million reads = 50,000 reads per MAG)
  phylo_expr_cov_pd <- rarefy_even_depth(phylo_expr_cov_pd, 
                                         sample.size = 0.05*min(min_reads$sample.sums),
                                         rngseed = 777)
  
  # print(sum(sample_sums(phylo_expr_cov_pd)))
  pd_tmp <- Phenoflow::Diversity_16S(phylo_expr_cov_pd, R = 10, brea = FALSE)
  pd_tmp <- data.frame(Sample = unique(expr_cov_pd$Sample)[i], 
                       Genome_ID = sample_names(phylo_expr_cov_pd),
                       pd_tmp)
...
# order genomes according to phylogeny
results_pd$new_bin_name <- factor(results_pd$new_bin_name, levels =
                                      c("MAG5.SP-M110-DD","MAG2.FA-MLB-SN",
                                        ...
                                        "MAG6.SP-M15-SD", "MAG8.SU-M110-DCMD",
                                        "MAG7.SU-MLB-SD","MAG9.SU-M15-SN"))
results_pd$Season <- factor(results_pd$Season, levels = c("Spring","Summer","Fall"))
results_pd$Depth <- factor(results_pd$Depth, levels = c("Surface","Mid","Deep"))
results_pd$Site <- gsub("110", "Lake Michigan\nsite M110", results_pd$Site)
results_pd$Site <- gsub("15", "Lake Michigan\nsite M15", results_pd$Site)
results_pd$Site <- gsub("Buoy", "Muskegon Lake", results_pd$Site)
results_pd$Site <- factor(results_pd$Site, levels = c("Muskegon Lake",
                                                            "Lake Michigan\nsite M15",
                                                            "Lake Michigan\nsite M110"))

# Run statistics for D2
for(i in 1:length(unique(results_pd$new_bin_name))){
  tmp_pd <- results_pd[results_pd$new_bin_name == unique(results_pd$new_bin_name)[i], ]
  # tmp_pd <- tmp_pd %>% dplyr::filter(Depth == "Surface")
  print(unique(results_pd$new_bin_name)[i])
  print(kruskal.test(D2 ~ Season,
             data = tmp_pd))
}
...
```{r PhenoD-2, dpi = 500, warning = FALSE, fig.width = 9, fig.height = 6, dev = c("png","pdf")}
# Plot results
...
```{r PhenoD-2b, dpi = 500, warning = FALSE, fig.width = 8.5, fig.height = 7}
...
# Posigene
```{r posigene, dpi = 500, warning = FALSE, fig.width = 7, fig.height = 5, include = FALSE}
# Import results file
data_posi <- read.table("./posigene/Limnohabitans_sp_Rim11_results_short.tsv",
                              header = TRUE,sep = "\t")

data_posi <- data_posi[, 1:5]
data_posi <- data.frame(Gene = rownames(data_posi), Transcript =
                          data_posi[,1],
                        FDR = data_posi[,2],
                        P.Value = data_posi[,3],
                        Number.of.species.included = data_posi[,4])

data_posi <- data_posi %>% dplyr::filter(FDR < 0.05 & P.Value < 0.05)

# Annotate PSGs
data_posi_annot <- dplyr::left_join(data_posi, MAG8_annot,
                                    by = c("Gene" = "gene_oid"))

```

# External metagenomes
```{r SDP_external, warning = FALSE, fig.width = 10, fig.height = 7, dpi = 500, dev = c("png", "pdf")}
...
```{r SDP_alignment-check-1, warning = FALSE, fig.width = 10, fig.height = 7, dpi = 500, dev = c("png", "pdf")}
