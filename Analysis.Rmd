...
# Perform PCoA on gene-length normalize data (RPK scaling)
pcoa_exp_table <- expr_cov_long %>% 
  dplyr::mutate(unique_id = interaction(Genome_ID, Sample, sep = "-")) %>% 
  dplyr::filter(!is.na(ko_id)) %>% 
  ungroup() %>% 
  dplyr::select(unique_id, ko_id, TPM) %>% 
  distinct() %>% 
  tidyr::spread(ko_id, TPM)
rownames(pcoa_exp_table) <- pcoa_exp_table$unique_id

# scale input data
pcoa_exp_res <- cmdscale(dist(scale(pcoa_exp_table[-1])), k =2, eig = TRUE)
var_exp <- round(pcoa_exp_res$eig/sum(pcoa_exp_res$eig)*100,1)
pcoa_exp_res <- data.frame(unique_id = rownames(pcoa_exp_table), pcoa_exp_res$points)
pcoa_exp_res <- pcoa_exp_res %>% 
  separate(unique_id, sep = "-", into = c("Genome_ID", "Sample_ID")) %>% 
  left_join(., meta_metaT, by = "Sample_ID")
...
pcoa_exp_ord <- ggplot(data=pcoa_exp_res, aes(x=X1, y=X2, shape = Site))+
  scale_shape_manual(values=c(21,23, 24))+
  geom_point(size=7, alpha=0.7, aes(fill = Genome_ID))+
  theme_bw()+
  scale_fill_brewer(palette = "Paired")+
  labs(x = paste0("PCoA axis 1 (",var_exp[1], "%)"),
       y = paste0("PCoA axis 2 (",var_exp[2], "%)"), fill="", shape = "", colour="")+
  ...
  ggtitle("PCoA of gene expression")

png(paste0("./Figures/metaT_overview_PCOA.png"), height = 6,
  width = 10, res = 500, units = "in")
print(pcoa_exp_ord)
dev.off()

# For each MAG calculate the dissimilarities in expression profiles between samples
