...
meta <- read.csv2("./Metadata/metadata.csv")
...
# read file
data.ANI <- read.table("./Dereplication/ANI/ANIb_percentage_identity.tab")

# Replace genome names by better annotated names
map_ani <- read.delim("./Dereplication/ANI/pyani_ref_names.tsv", stringsAsFactors = FALSE)
...
## Investigate predicted generation time (MGT)
...
## Network analysis based on 16S data
...
## From mapped reads info (BBmap - `pileup.sh`)  
...
#   distinct()
pca_dgelist <- edgeR::DGEList(counts = pca_exp_table)
pca_norm <- edgeR::calcNormFactors(pca_dgelist, method = "TMMwsp")
pca_norm <- edgeR::cpm(pca_norm, log = TRUE)

# Perform PCA
pca_exp_res <- prcomp(pca_exp_table, center = TRUE, scale = TRUE)

# variance explained
eigs <- pca_exp_res$sdev^2

# Store results
if (i == 1) {
  results_PCA <- data.frame(
    Sample = rownames(pca_exp_res$x),
    pca_exp_res$x,
  ...
  results_PCA_var <-
    data.frame(
    Genome = levels(expr_cov_long$Genome_ID)[i],
    Var = eigs[1:2] / sum(eigs),
    PC = colnames(pca_exp_res$x)[1:2]
    )
} else {
  results_PCA <- rbind(results_PCA,
    data.frame(
    Sample = rownames(pca_exp_res$x),
    pca_exp_res$x,
    Genome = levels(expr_cov_long$Genome_ID)[i]
  ...
  results_PCA_var <- rbind(results_PCA_var,
    data.frame(
    Genome = levels(expr_cov_long$Genome_ID)[i],
    Var = eigs[1:2] / sum(eigs),
    PC = colnames(pca_exp_res$x)[1:2]
...
results_PCA_var <- left_join(results_PCA_var, new_bin_names2, 
                          by = c("Genome" = "IMG_taxID"))
# Add metadata
...
p_exp_PCA <- results_PCA %>%   
  dplyr::filter(Depth == "Surface" & 
      !new_bin_name %in% c("MAG5.SP-M110-DD", "MAG2.FA-MLB-SN")) %>%
  ggplot(aes(x = PC1, y = PC2, fill = Season, shape = Site))+
  scale_shape_manual("", values = c(21, 22, 25))+
  geom_point(size = 3.5, alpha = 0.8)+
  facet_wrap(.~new_bin_name, ncol = 4)+
...
png("./Figures/Revisions/PCA_expression1.png", width = 12, height = 6, units = "in", res = 500)
print(p_exp_PCA)
dev.off()
 
svg("./Figures/Revisions/PCA_expression1.svg", width = 12, height = 6)
print(p_exp_PCA)
dev.off()

p_exp_PCA2 <- results_PCA %>%   
  dplyr::filter(Site == "Lake Michigan\nsite M110" & Depth != "Mid" & Season != "Spring" &
      !new_bin_name %in% c("MAG5.SP-M110-DD", "MAG2.FA-MLB-SN")) %>%
...
png("./Figures/Revisions/PCA_expression2.png", width = 12, height = 6, units = "in", res = 500)
print(p_exp_PCA2)
dev.off()

p_exp_PCA3 <- results_PCA %>%   
  dplyr::filter(Depth == "Surface" & 
      !new_bin_name %in% c("MAG5.SP-M110-DD", "MAG2.FA-MLB-SN")) %>%
  ggplot(aes(x = Site, y = PC1))+
  geom_point(size = 3.5, alpha = 0.8, aes(shape = Site, fill = Season))+
    scale_shape_manual("", values = c(21, 22, 25))+
...
png("./Figures/Revisions/PCA_expression3.png", width = 12, height = 6, units = "in", res = 500)
print(p_exp_PCA3)
dev.off()


p_exp_PCA4 <- results_PCA %>%   
  dplyr::filter(Depth == "Surface" & 
      !new_bin_name %in% c("MAG5.SP-M110-DD", "MAG2.FA-MLB-SN")) %>%
  ggplot(aes(x = Site, y = PC2))+
  geom_point(size = 3.5, alpha = 0.8, aes(shape = Site, fill = Season))+
    scale_shape_manual("", values = c(21, 22, 25))+
  geom_boxplot(outlier.shape = NA, alpha = 0.5, width = 0.3)+
  facet_wrap(.~new_bin_name, ncol = 4)+
...
png("./Figures/Revisions/PCA_expression4.png", width = 12, height = 6, units = "in", res = 500)
print(p_exp_PCA4)
dev.off()

png("./Figures/Revisions/PCA_expression34.png", width = 12, height = 12, units = "in", res = 500)
cowplot::plot_grid(p_exp_PCA3, p_exp_PCA4, ncol = 1, align = "hv" )
dev.off()
...
res_deseq_anott %>% 
  dplyr::select(gene_oid, log2FoldChange, new_bin_name, Comparison, Design, padj) %>% 
  dplyr::filter(Design == "~ Site + Season" 
                & Comparison == "Fall-Spring" & is.na(padj)) %>% 
    distinct() %>% 
    group_by(new_bin_name) %>% 
  summarise(nrow = length(unique(gene_oid)))
...
p_deseq_2 <- res_deseq_anott %>% 
  dplyr::filter(!is.na(ko_id) & ko_level_B %in% selected_ko & Design == "~ Site + Season") %>% 
  dplyr::mutate(new_bin_name = factor(new_bin_name, levels =
                                      c("MAG5.SP-M110-DD","MAG2.FA-MLB-SN",
                                        "MAG3.FA-MLB-SN", "MAG4.FA-M110-DN",
                                        "MAG1.FA-MLB-DN", "MAG10.SU-M15-SN",
                                        "MAG6.SP-M15-SD", "MAG8.SU-M110-DCMD",
                                        "MAG7.SU-MLB-SD","MAG9.SU-M15-SN"))) %>% 
  dplyr::filter(!new_bin_name %in% c("MAG2.FA-MLB-SN", "MAG5.SP-M110-DD")) %>%
  dplyr::filter(Comparison == "Spring-Summer") %>% 
  dplyr::filter(log2FoldChange > 2) %>% 
  ggplot2::ggplot(aes(x = new_bin_name, y = ko_level_C, fill = ko_level_B))+
  # geom_point(size = 4, shape = 21)+
  ggplot2::geom_count(color = "black", shape = 21)+
  scale_fill_brewer(palette = "Set2")+
  scale_size("Number of genes", range = c(3.5,8), breaks=seq(1,12,by=3)) +
  # ggplot2::scale_fill_distiller("log2FoldChange",palette="RdBu", na.value="white",
  # limits = c(-10,10)) +
  theme_bw()+
  ylab("")+
  xlab("")+
...
png("./Figures/Revisions/Figure4.png", width = 9, height = 8, units = "in", res = 500)
print(p_deseq_2)
dev.off()
...
res_deseq_anott_dom1 <- res_deseq_anott %>% 
  dplyr::filter(!is.na(ko_id) & ko_level_B %in% selected_ko & Design == "~ Site + Season") %>% 
  dplyr::mutate(new_bin_name = factor(new_bin_name, levels =
                                      c("MAG5.SP-M110-DD","MAG2.FA-MLB-SN",
                                        "MAG3.FA-MLB-SN", "MAG4.FA-M110-DN",
                                        ...
                                        "MAG7.SU-MLB-SD","MAG9.SU-M15-SN"))) %>% 
  dplyr::filter(!new_bin_name %in% c("MAG2.FA-MLB-SN", "MAG5.SP-M110-DD")) %>%
  dplyr::filter(Comparison == "Fall-Spring") %>% 
  # dplyr::filter(log2FoldChange > 1) %>%
  dplyr::filter(cog_id %in% DOM_usage_df$COG_ID) %>% 
  left_join(., DOM_usage_df, by = c("cog_id" = "COG_ID"))

# Order DOC categories
COG_order1 <- DOM_usage_df %>% 
  dplyr::filter(COG_ID %in% unique(res_deseq_anott_dom1$cog_id))
res_deseq_anott_dom1$cog_id <- factor(res_deseq_anott_dom1$cog_id,
                                        levels = COG_order1$COG_ID)
p_deseq_dom_1 <-  res_deseq_anott_dom1 %>% 
  ggplot2::ggplot(aes(x = new_bin_name, y = cog_id, fill = DOM_type))+
  # geom_point(size = 4, shape = 21)+
  ggplot2::geom_count(color = "black", shape = 21)+
  scale_fill_brewer("", palette = "Set3")+
  scale_size("Number of genes", range = c(3.5,8), breaks=seq(1, 7,by=2)) +
  # ggplot2::scale_fill_distiller("log2FoldChange",palette="RdBu", na.value="white",
  # limits = c(-10,10)) +
  theme_bw()+
  ylab("")+
...
png("./Figures/Revisions/Figure_deseq_dom_1.png", width = 7, height = 6, units = "in", res = 500)
print(p_deseq_dom_1)
dev.off()

res_deseq_anott_dom2 <- res_deseq_anott %>% 
  dplyr::filter(!is.na(ko_id) & ko_level_B %in% selected_ko & Design == "~ Season + Site") %>% 
  dplyr::mutate(new_bin_name = factor(new_bin_name, levels =
                                      c("MAG5.SP-M110-DD","MAG2.FA-MLB-SN",
                                        "MAG3.FA-MLB-SN", "MAG4.FA-M110-DN",
                                        ...
                                        "MAG7.SU-MLB-SD","MAG9.SU-M15-SN"))) %>% 
  dplyr::filter(!new_bin_name %in% c("MAG2.FA-MLB-SN", "MAG5.SP-M110-DD")) %>%
  dplyr::filter(Comparison == "Muskegon Lake\nVs\nM110") %>% 
  # dplyr::filter(log2FoldChange > 1) %>%
  dplyr::filter(cog_id %in% DOM_usage_df$COG_ID) %>% 
  left_join(., DOM_usage_df, by = c("cog_id" = "COG_ID"))

  # Order DOC categories
COG_order2 <- DOM_usage_df %>% 
  dplyr::filter(COG_ID %in% unique(res_deseq_anott_dom2$cog_id))
res_deseq_anott_dom2$cog_id <- factor(res_deseq_anott_dom2$cog_id,
                                        levels = COG_order2$COG_ID)

p_deseq_dom_2 <- res_deseq_anott_dom2 %>% 
  ggplot2::ggplot(aes(x = new_bin_name, y = cog_id, fill = DOM_type))+
  # geom_point(size = 4, shape = 21)+
  ggplot2::geom_count(color = "black", shape = 21)+
  scale_fill_brewer("", palette = "Set3")+
...
png("./Figures/Revisions/Figure_deseq_dom_2.png", width = 7, height = 6, units = "in", res = 500)
print(p_deseq_dom_2)
dev.off()

cowplot::plot_grid(p_deseq_dom_1, p_deseq_dom_2, align = "hv", ncol = 1)
```
...
# Export results
res_deseq_anott_changed_toCSV <- res_deseq_anott_changed %>% 
  # select(1:9, 12, 13,15) %>% 
  distinct()  
res_deseq_anott_changed_toCSV <- apply(res_deseq_anott_changed_toCSV, 2, gsub, patt="\n", replace=";")

write.csv2(res_deseq_anott_changed_toCSV, file = "./Manuscript/SupplementaryData_DESeq_1.csv")
...
png("./Figures/Revisions/Figure3b.png", width = 7, height = 5, units = "in", res = 500)
print(p_deseq_overview2)
dev.off()

svg("./Figures/Revisions/Figure3b.svg", width = 7, height = 5)
print(p_deseq_overview2)
dev.off()
...
blast_df_sum$new_bin_name <- factor(blast_df_sum$new_bin_name, levels =
                                      c("MAG5.SP-M110-DD","MAG2.FA-MLB-SN",
                                        ...
                                        "MAG6.SP-M15-SD", "MAG8.SU-M110-DCMD",
                                        "MAG7.SU-MLB-SD","MAG9.SU-M15-SN"))
# Plot for most abundant bin (MAG8)
